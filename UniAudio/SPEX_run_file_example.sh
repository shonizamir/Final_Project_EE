    ...
    for part in $valid_set $train_set; do
      for n in `seq 0 $[$ngpu-1]`; do
        ...
        python3 data_scripts/create_data_json.py \
         --task Spex \
         --out-json   $PWD/data/${part}/${ngpu}splits/data_spex.${n}.json \
...
train_data_jsons="data/${train_set}/${ngpu}splits/data_spex.0.json"
valid_data_jsons="data/${valid_set}/${ngpu}splits/data_spex.0.json"
    ...
    for part in $vc_test_sets; do
        ...
        utils/run.pl --max-jobs-run 8 JOB=0:$[${ngpu}-1] \
          ...
          python3 ../../infer.py \
            ...
            --seed 888 \
            --rank JOB \
	          --data_json /storage/shoni/UniAudio/UniAudio/egs/SPEX/data/val/1splits/${data_json} \
