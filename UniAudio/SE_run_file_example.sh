...
seed=999
debug=false
batch_scale=2000 # the total number of tokens in one batch
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for part in $test_set $valid_set $train_set; do
      mkdir -p data/${data_tag}/${part}/${ngpu}splits
      # extra shuf to ensure balance across GPUs
      # So the generated data cannot be reproduced due to the shuffle randomness
      cat data/${data_tag}/${part}/wav.scp | shuf >  data/${data_tag}/${part}/wav.scp.shuf
      split_scp=
      for n in `seq 1 $ngpu`; do
          split_scp="$split_scp data/${data_tag}/${part}/${ngpu}splits/wav.${n}.scp"
      done
      utils/split_scp.pl data/${data_tag}/${part}/wav.scp.shuf $split_scp
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for part in $valid_set $train_set $test_set; do
    # for part in $valid_set; do
      echo "prepare $part ... "

      # split noise.scp based on clean.scp
      utils/run.pl JOB=1:$ngpu data/${data_tag}/${part}/${ngpu}splits/log/filter_noise.JOB.log \
        python3 data_scripts/filter_scp.py \
          data/${data_tag}/${part}/${ngpu}splits/wav.JOB.scp data/${data_tag}/${part}/noise.scp \
          data/${data_tag}/${part}/${ngpu}splits/noise.JOB.scp || exit 1;

      # clean Audio
      utils/run.pl JOB=1:$ngpu data/${data_tag}/${part}/${ngpu}splits/log/audio_codec_dump.JOB.log \
        python3 data_scripts/offline_tokenization.py \
          --input-file data/${data_tag}/${part}/${ngpu}splits/wav.JOB.scp \
          --output-file data/${data_tag}/${part}/${ngpu}splits/clean_codec.JOB.pt \
          --tokenizer audio --rank JOB || exit 1;

      # noise Audio
      utils/run.pl JOB=1:$ngpu data/${data_tag}/${part}/${ngpu}splits/log/noise_codec_dump.JOB.log \
        python3 data_scripts/offline_tokenization.py \
          --input-file data/${data_tag}/${part}/${ngpu}splits/noise.JOB.scp \
          --output-file data/${data_tag}/${part}/${ngpu}splits/noise_codec.JOB.pt \
          --tokenizer audio --rank JOB || exit 1;
    ...
    for part in $valid_set $train_set  $test_set; do
      for n in `seq 0 $[$ngpu-1]`; do
        ...
        python3 data_scripts/create_data_json.py \
         --task SE \
         --out-json   $PWD/data/${data_tag}/${part}/${ngpu}splits/data_se.${n}.json \
         --noise_seq  $PWD/data/${data_tag}/${part}/${ngpu}splits/noise_codec.$[$n+1].pt \
         --audio_seq  $PWD/data/${data_tag}/${part}/${ngpu}splits/audio_codec.$[$n+1].pt \
         & 
      done; wait
...
train_data_jsons="data/${data_tag}/${train_set}/${ngpu}splits/data_se.0.json"
valid_data_jsons="data/${data_tag}/${valid_set}/${ngpu}splits/data_se.0.json"
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    for part in $vc_test_sets; do
        ...
        data_json="data_se.JOB.json" # your val set .json file
        ...
        utils/run.pl --max-jobs-run 8 JOB=0:$[${ngpu}-1] \
          ...
          python3 ../../infer.py \
            ...
            --seed 888 \
            --rank JOB \
	          --data_json /storage/shoni/UniAudio/UniAudio/egs/SE/data/simplex_final_5s/test/1splits/${data_json} \
